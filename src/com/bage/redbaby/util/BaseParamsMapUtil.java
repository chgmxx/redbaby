...
public class BaseParamsMapUtil {
	...
	public static Map<String, String> getParamsMap(Context context) {
		...
		paramsMap.put("udid", "udid or imei");
		paramsMap.put("os", "Android os");
		paramsMap.put("osversion", "5.0");
		paramsMap.put("appversion", "1.0.0");
		paramsMap.put("Sourest", "Yek_test");
		paramsMap.put("ver", "0.9");
		paramsMap.put("userid", "12345");
		paramsMap.put("usersession", "Cbaq4fxvb");
		paramsMap.put("unique", "xvbvsfsgdsg");
		if (imei == null) {
		...
		if (cid == null) {
			try {
				...
				Object i = metaData.get("UMENG_CHANNEL");
				cid = String.valueOf(i);
			} catch (Exception e) {
				cid = "" + -1;
				e.printStackTrace();
			}
